...
# Function to normalize strings (remove accents, convert to uppercase)
def normalize_string(s):
    return unidecode.unidecode(s).upper()

# Create a dictionary mapping COMUNA to REGION
comuna_to_region = {}
for region_entry in region_data['regiones']:
    region_name = region_entry['region']
    for comuna in region_entry['comunas']:
        comuna_to_region[normalize_string(comuna)] = region_name

# Read the orders.csv file
orders_file = "orders.csv"  # Replace with your file path
df = pd.read_csv(orders_file)

# Print column names for debugging
print("Columns in CSV:", df.columns)

# Normalize the COMUNA column for matching
df['COMUNA_NORMALIZED'] = df['Comuna'].apply(normalize_string)

# Map the normalized COMUNA to REGION
df['Region'] = df['COMUNA_NORMALIZED'].map(comuna_to_region)

# Drop the normalized column (optional)
df.drop(columns=['COMUNA_NORMALIZED'], inplace=True)

# Save the updated DataFrame to a new CSV file
df.to_csv("orders_with_regions.csv", index=False)

print("Mapping completed. Updated file saved as 'orders_with_regions.csv'.")
